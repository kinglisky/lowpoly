...
var METHODS = {
	...
	//将图片数据分组执行执行
	shuntImageData: function (imageData) {
		var width = imageData.width | 0,
			height = imageData.height | 0,
			data = Array.prototype.slice.call(imageData.data),
			shuntHeight = 100,
			shuntNum = (height / shuntHeight) | 0,
			restHeight = height % shuntHeight,
			//每个分片数组的长度(4表示rgba)
			shuntFragment = width * shuntHeight * 4,
			start = 0,
			shuntData = [];
		for (var i = 0; i < shuntNum; i++) {
			shuntData.push({
				width: width,
				height: shuntHeight,
				data: data.slice(start, start += shuntFragment)
			});
		}
		shuntData.push({
			width: width,
			height: restHeight,
			data: data.slice(start, start + (width * restHeight) * 4)
		});
		return function (progress, end) {
			var index = 0;
			while (shuntData.length > 0) {
				progress(shuntData.shift(), index++);
			}
			end();
		}
	}
...
var To = {
	...
	do: function () {
		var set = BASE.set,
			...
			edge = BASE.edge,
			shuntHandle = null;
		shuntHandle = METHODS.shuntImageData(imgData);
		shuntHandle(function (imageData, index) {
			//过滤器用于处理图片的数据
